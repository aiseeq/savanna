...
const (
	...
	// Допуск для проверок насыщения (избегает проблем с float32)
	SatietyTolerance = 0.1 // 99.9% считается полной сытостью
)
...
const (
	...
	NutritionToHungerRatio = 0.5 // Эффективность превращения питательности в голод (50%)
...
const (
	...
	// Старые константы (DEPRECATED - больше не используются)
	SpeedVeryHungryMultiplier = 1.0 // Теперь голодные бегают с полной скоростью
	SpeedOverfedMultiplier    = 0.8 // Заменено на новую формулу
	VerySlowHungerThreshold   = 20.0 // Больше не используется
	OverfedHungerThreshold    = 80.0 // Заменено на OverfedSpeedThreshold
)
