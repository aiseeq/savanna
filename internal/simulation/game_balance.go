...
const (
	...
	// УМЕНЬШЕНИЕ В 10 РАЗ: для более реалистичных размеров животных
	// Заяц: был 0.5 тайла → теперь 0.05 тайла (1.6 пикселя после конвертации)
	// Волк: был 0.75 тайла → теперь 0.075 тайла (2.4 пикселя после конвертации)
	RabbitBaseRadius = 0.05  // Заяц - радиус 0.05 тайла → 1.6 пикселя после конвертации
	WolfBaseRadius   = 0.075 // Волк - радиус 0.075 тайла → 2.4 пикселя после конвертации
...
const (
	...
	// УМЕНЬШЕНИЕ В 20 РАЗ: для очень медленного, наблюдаемого движения
	RabbitBaseSpeed = 0.15 // Заяц: 0.15 тайла/сек (было 3.0 тайла/сек)
	WolfBaseSpeed   = 0.2  // Волк: 0.2 тайла/сек (было 4.0 тайла/сек, может догнать зайца)
